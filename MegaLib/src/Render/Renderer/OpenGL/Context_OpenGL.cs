...
namespace MegaLib.Render.Renderer.OpenGL
{
  ...
  public class TT
  {
    public RO_Base Object;
    private Context_OpenGL _context;
    private OpenGL_Shader _shader;

    public List<string> BufferList = new();
    public List<string> TextureList = new();
    public Dictionary<string, Func<dynamic>> UniformList = new();
    ...
    public TT(OpenGL_Shader shader, Context_OpenGL context, RO_Base obj)
    {
      _context = context;
      Object = obj;
      _shader = shader;
    ...
    public void Init()
    {
      ...
      foreach (var tuple in BufferList.Select(t => t.Split(" -> ")))
      {
        ...
        if (name == "index")
        {
          var list = (List<uint>)Object.GetDataByName(RO_DataType.Buffer, name);
        ...
        {
          var list = (List<Vector2>)Object.GetDataByName(RO_DataType.Buffer, name);
        ...
        {
          var list = (List<Vector3>)Object.GetDataByName(RO_DataType.Buffer, name);
      ...
      foreach (var tuple in TextureList.Select(t => t.Split(" -> ")))
      {
        ...
        // Create texture
        var texture = (Texture_Base)Object.GetDataByName(RO_DataType.Texture, name);
        if (texture == null) continue;
        glCreateTexture(name, texture.GPU_RAW, texture.Options);
  ...
  public class Context_OpenGL
  {
    ...
    public void BindMatrix(string shaderProgram, string varName, Matrix4x4 matrix)
    {
      ...
      OpenGL32.glUniformMatrix4fv(uniformLocation, 1, 0, matrix.Raw);
    }
    ...
    public void ActivateCubeTexture(OpenGL_Shader shader, string varName, string textureName, uint slot)
    {
      // Activate slot texture
      OpenGL32.glActiveTexture(OpenGL32.GL_TEXTURE0 + slot);
      ...
      // Set shader.varName = slot;
      OpenGL32.glUniform1i(shader.GetUniformLocation(varName), (int)slot);
    }
  }
}